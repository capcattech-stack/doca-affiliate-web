...
export function initNavbarAuth() {
  ...
  if (!client) {
    console.warn('[NavbarAuth] Supabase client không khả dụng.');
    return;
  }
  ...
  async function renderAuthUI(user: any) {
    if (user) {
      ...
      const btnHtml = `
        <button class="nav-google-btn-circular" title="Kết nối tài khoản Google">
      ...
      // Nút trên Mobile: nút dài có chứa logo Google 4 màu
      const mobileBtnHtml = `
